...
function form_onSubmit(e) {
  ...
  let id = document.getElementById("ownerID").value;
  ...
  if (document.getElementById("email").value.length > 0) {
    email_check = check_email(document.getElementById("email").value, id);
  }
  ...
  Promise.all([username_check, email_check]).then(
    ([username_check, email_check]) => {
      ...
      if (!errors) {
        let result = fetch("./edit_user.php", {
          method: "POST",
        ...
        //     window.location.href = "./admin_dashboard.php?status=success";
        //   } else {
        //     window.location.href = "./add_user.php?status=failure";
        //   }
        // });
      }
    }
 ...
 */
async function check_username(username, ownerID) {
  let valid_username = await fetch(
    `./../api/validate_username.php?username=${username}&ownerID=${ownerID}`,
    {
      mode: "cors",
      headers: {
 ...
 */
async function check_email(email, ownerID) {
  let valid_email = await fetch(
    `./../api/validate_email.php?email=${email}&ownerID=${ownerID}`,
    {
      mode: "cors",
      headers: {
        "Content-Type": "application/json",
