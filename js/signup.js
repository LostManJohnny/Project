					...
					switch (id) {
						...
						case "fname":
							if (input.value.length == 0) {
								...
								special_characters.test(input.value)
							) {
								error_text =
						...
						case "birthdate":
							if (input.value.length == 0) {
								error_text =
									"Birthdate name cannot be empty";
							} else if (
						...
						case "password":
							...
							if (input.value == "") {
								...
								error_text =
									"Password cannot exceed 45 characters long";
				...
				result.then((result) => {
					if (result["status"] == 200) {
						window.location.href =
							"./signup_results.php?status=success";
					} else {
						window.location.href =
							"./signup_results.php?status=failure";
					}
				});
			}
		}
...
async function check_email(email) {
	let valid_email = await fetch(
		`./api/validate_email.php?email=${email}`,
		{
			mode: "cors",
			headers: {
				"Content-Type": "application/json",
